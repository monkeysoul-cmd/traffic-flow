...
const VehicleDetectionSchema = z.object({
  box: z
    .array(z.number())
    .describe(
      "The bounding box of the detected vehicle [x, y, width, height].",
    ),
  timestamp: z
    .number()
    .describe("The timestamp in seconds when the vehicle was detected."),
});
...
const AnalyzeTrafficDataOutputSchema = z.object({
  vehicleCount: z
    .number()
    .describe(
      "The total number of unique vehicles detected in the camera feed.",
    ),
  trafficLevel: z
    .string()
    .describe(
  ...
  vehicles: z
    .array(VehicleDetectionSchema)
    .describe(
      "A list of all detected vehicles with their bounding boxes and timestamps.",
    ),
});

export async function analyzeTrafficData(input) {
